...
func LoginHandler(c *gin.Context, appid, secret string, mongo_client *mongo.Client) {
    // 获取请求参数 wxcode
    wxcode := c.Query("wxcode")
    if wxcode == "" {
        // 如果 wxcode 为空，返回错误响应
        utils.HandleError(c, http.StatusBadRequest, errMsgInvalidParams, nil)
        return
    }
    ...
    client := &http.Client{}
    // 构建请求 URL
    url := fmt.Sprintf("%s?appid=%s&secret=%s&js_code=%s&grant_type=authorization_code", loginURL, appid, secret, wxcode)
    // 创建 HTTP GET 请求
    req, err := http.NewRequest("GET", url, nil)
    if err != nil {
        // 如果创建请求失败，返回错误响应
        utils.HandleError(c, http.StatusInternalServerError, errMsgInternalError, err)
        return
    }
    ...
    collection := mongo_client.Database("serenesong").Collection("users")
